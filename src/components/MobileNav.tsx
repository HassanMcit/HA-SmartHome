...
export default function MobileNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="mx-4 mb-4 bg-[#070a13]/90 backdrop-blur-xl border border-white/5 rounded-2xl shadow-2xl shadow-black/50 p-2 flex items-center justify-around">
        {mainItems.map((item) => {
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent side="bottom" className="bg-[#070a13] border-white/5 rounded-t-[32px] p-0 outline-none max-h-[85dvh] overflow-hidden flex flex-col">
            {/* Handle bar + header */}
