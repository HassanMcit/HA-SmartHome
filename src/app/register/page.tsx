...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-[#0f0f23] relative overflow-hidden selection:bg-sky-500/30">
      {/* Background orbs */}
      <div className="absolute top-[-5%] right-[10%] w-[250px] h-[250px] sm:w-[450px] sm:h-[450px] rounded-full bg-sky-600/10 blur-[70px] sm:blur-[110px] pointer-events-none animate-pulse" />
        ...
        <div className="glass-card p-6 sm:p-10 border-white/5">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="h-12 bg-[#242444] border-[#2d2d5e] text-slate-200 placeholder:text-slate-500 focus:border-sky-500 focus:ring-sky-500/20 rounded-xl"
                disabled={loading}
              />
            </div>
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-12 sm:h-14 text-base sm:text-lg font-bold rounded-xl text-white bg-gradient-to-r from-sky-600 to-purple-600 hover:from-sky-500 hover:to-purple-500 shadow-lg shadow-sky-500/20 transition-all active:scale-[0.98] mt-4 disabled:opacity-70 disabled:active:scale-100"
            >
              {loading ? (
          ...
          <div className="mt-8 pt-8 border-t border-white/5 text-center">
            <p className="text-slate-400 text-sm">
              لديك حساب بالفعل؟{' '}
              <Link href="/login" className="text-sky-400 font-bold hover:text-sky-300 transition-colors underline-offset-4 hover:underline">
                تسجيل الدخول
              </Link>
            </p>
          </div>
